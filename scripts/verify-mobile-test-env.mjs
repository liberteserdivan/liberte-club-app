#!/usr/bin/env node
/**
 * Mobil smoke test on kosul kontrolu — secret degerleri loglanmaz.
 */
const REQUIRED = [
...
const OPTIONAL = ['MOBILE_TEST_ADMIN_PIN'];
...
/** Env anahtari dolu mu (deger loglanmaz) */
...
function main() {
  ...
  if (missing.length) {
    ...
    console.error('Codemagic -> liberte_android grubuna ekleyin (mobile_device_tests opsiyonel).');
    process.exit(1);
  }
  ...
  if (!hasApp) {
    ...
    console.error('Alternatif: MOBILE_ANDROID_APK_PATH veya BROWSERSTACK_APP_ANDROID_URL');
    process.exit(1);
  }
