...
// Public push g├Ârseli (FCM imageUrl) ÔÇö ayr─▒ serverless function yok (Hobby limiti)
async function handlePushMedia(req, res) {
  ...
  try {
    ...
    if (!row) {
      return res.status(404).json({ error: 'G├Ârsel bulunamad─▒' });
    }
    ...
    return res.status(500).json({ error: 'G├Ârsel okunamad─▒' });
  }
}
...
export default async function pushRouter(req, res) {
  ...
  return res.status(400).json({ error: 'Ge├ğersiz push action' });
}
