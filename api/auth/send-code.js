...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (purpose === 'register') {
      ...
      return res.status(400).json({ error: 'Giriş için /api/auth/login kullanın' });
    }
    ...
    await sql`INSERT INTO email_codes (email, phone, code, purpose, expires_at)
      VALUES (${email}, ${phone}, ${code}, 'register', now() + interval '10 minutes')`;
    await sql`UPDATE email_codes SET used=true
      WHERE email=${email} AND phone=${phone} AND purpose='register' AND code<>${code} AND used=false`;

    const apiKey = process.env.RESEND_API_KEY;
    const from = process.env.RESEND_FROM_EMAIL || 'Liberte <noreply@liberte.cafe>';
    const subject = 'Liberte kayıt kodun';
    const html = `<div style="font-family:Arial,sans-serif;background:#06110d;color:#fff;padding:28px;border-radius:18px"><h2 style="color:#b9f5d0">Liberte</h2><p>Merhaba ${name},</p><p>Kayıt doğrulama kodun:</p><div style="font-size:34px;letter-spacing:8px;font-weight:800;color:#b9f5d0;margin:18px 0">${code}</div><p>Bu kod 10 dakika geçerlidir.</p></div>`;
