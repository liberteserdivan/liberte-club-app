...
export async function handleAdminLoyaltyAction(req, res) {
  ...
  try {
    ...
    // Üye tabloda varsa relational yol (flag kapalı kalsa bile app_state boş cutover)
    const sql = getSql();
    const relationalCustomer = sql
      ? await runSql(() => findCustomerById(sql, verified.customerId))
      : null;
    const useRelational = useRelationalState() || Boolean(relationalCustomer);
